...
function login_action() {
    ...
    // Acepta password por JSON y también por POST tradicional (por si el server toca algo)
    $password = trim((string)($data['password'] ?? ($_POST['password'] ?? '')));
    ...
    if ($hash === '' || !is_string($hash)) {
        echo json_encode([
            "success" => false,
            "error" => "APP_PASS_HASH_no_definido_en_config"
        ]);
        return;
    }

    $ok = ($password !== '') ? password_verify($password, $hash) : false;

    echo json_encode([
        "success" => (bool)$ok,
        "error"   => $ok ? "" : "password_incorrecta"
    ]);
}
